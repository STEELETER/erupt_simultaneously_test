...
from modules.utils.conf import updown_serv_addr, sysnum, trans_per_size
from modules.utils.sock import Soc_client

'''
'{"sysnum":"868608","id":"xxxxx","down_from":"xxxx","down_to":"xxxxxxx"}'
...
class Downloader():
    ...
    #线程管理，方便实现挂起和恢复
     ...
     def __check_file(self):
         #发送
         checkInfo = json.dumps({'sysnum':sysnum,'order':'down','fpath':self.down_from['fpath'],'has_down':self.down_to['has_down']})
         print("发送到服务器的文件验证信息：%s"%str(checkInfo))
         self.__client.sendall(checkInfo.encode())

         #接收
         recv_data = self.__client.recv(trans_per_size)  # 接收已经传了+多少
         l = len(recv_data)
         print("长度是：%s"%str(l))
         recv_data = recv_data.decode()
         print('接收到的服务器返回验证返回信息：%s'%str(recv_data))
